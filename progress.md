...
- 完整 smoke 已通过全部事件断言，随后停在既有状态栏计数断言：当前 DOM 有 22 个 `data-status-key`，旧测试仍期望 18；该问题早于本次事件修改且未在本任务中扩大范围处理。
...
- 通用网页游戏客户端无控制台错误；完整 smoke 通过本次配色覆盖的布局检查，之后仍停在既有的 22/18 状态栏计数断言。
