...
优先级 比赛结尾 > 自己 > 系列赛 > 对手，**每场最多 3 句**（超了按优先级截断）。
文案在比赛生成时就算好存进 `gameEntry.narrative`（重绘不会换句），然后：

- 季后赛：显示在单场简报里（未命中时不会凭空出句）
- 常规赛：进点阵图每个点的 `title`（悬停即看）+ 本季高光区块
- 三双文案仍由 `getTripleChaseNote` 提供，与上面共用同一次渲染

`tests/post-game-lines.test.js`（14 项）守住每种情况的判定边界，
包括“没给 boxScore 就不许提具体对手”和“最多 3 句”。
