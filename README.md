...
优先级 比赛结尾 > 自己 > 系列赛 > 对手，上限 3 句。
文案在比赛生成时就算好存进 `gameEntry.narrative`（重绘不会换句），然后：

- **常规赛场次**：进点阵图每个点的 `title`（悬停即看）+ 本季高光区块
- **季后赛**：显示在单场简报里
- **常规赛高光弹窗**：见下，与剧情事件共用 `showEventModal`

### 常规赛弹窗：事实层与播报层分开

这是两件事，不要搞混——**事实该记就记，弹窗才有概率**：

1. **事实层（不掷骰子）**：真发生了大场面（绝杀 / 加时绝杀 / 被反绝杀 /
   双加时 / 三双 / 40+ / 爆冷），就写进 `gameEntry.narrative`、点阵图 `title`、本季高光，**一场不漏**。
2. **播报层（掷骰子）**：值不值得停下来弹窗。常规赛本来就要快，
   所以 `REGULAR_SPOTLIGHT.chance = 0.3`，再叠冷却 3 场、每季上限 8 次。
   概率没过不消耗冷却（否则跳过的场次会白白吃掉下次机会）。

调“多久弹一次”只改 `REGULAR_SPOTLIGHT`，不要去砍文案——
文案变短和弹窗变少是两件不同的事（前者会让记录失真，后者才是流畅度）。
三双文案（`getTripleChaseNote`）也并进同一个弹窗。

`tests/post-game-lines.test.js`（23 项）守住：每种情况的判定边界、
“没给 boxScore 就不许提具体对手”、上限 3 句、弹窗字段完整、
事实层不因不弹窗而消失、概率不消耗冷却、冷却与每季上限、
以及常规赛与季后赛句数一致。
